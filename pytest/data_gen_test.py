...
class DataGenTest(unittest.TestCase):
    ...
    def test_marginal_data(self):
        ...
        np.testing.assert_allclose(
            data_v2[idx].mean(axis=0), data_v1[idx].mean(axis=0), rtol=rtol
        )
        np.testing.assert_allclose(data_v2[idx].mean(axis=0), [900, 2.025], rtol=rtol)
        self.assertRaises(
            AssertionError,
            np.testing.assert_allclose,
            actual=data_v2[idx].mean(axis=0),
            desired=[1000, 2.25],
            rtol=rtol,
        )
        ...
        rtol = 0.03
        np.testing.assert_allclose(
            data_v2[idx].mean(axis=1), data_v1[idx].mean(axis=1), rtol=rtol
        )
        np.testing.assert_allclose(data_v2[idx].mean(axis=1), [1, 1], rtol=rtol)
        self.assertRaises(
            AssertionError,
            np.testing.assert_allclose,
            actual=data_v2[idx].mean(axis=1),
            desired=[2, 2],
            rtol=rtol,
        )
    ...
    def test_strategic_variables(self):
        ...
        idx = 4
        np.testing.assert_allclose(
            data_v2[idx].mean(axis=1), data_v1[idx].mean(axis=1), rtol=0.3
        )
        np.testing.assert_allclose(
            data_v2[idx].mean(axis=1), [[1, 1], [1, 2]], rtol=0.2
        )
        np.testing.assert_allclose(
            data_v1[idx].mean(axis=1), [[1, 1], [1, 2]], rtol=0.2
        )
        self.assertRaises(
            AssertionError,
            np.testing.assert_allclose,
