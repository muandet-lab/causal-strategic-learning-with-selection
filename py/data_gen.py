...
def generate_theta(
    i: int, args: argparse.Namespace, deploy_sd_every: int
) -> np.ndarray:
    ...
    if args.scaled_duplicates is None:  # random vectors for every round
        ...
        args.scaled_duplicates == "sequence"
    ):  # making sure there exists a scaled duplicate of each theta per round
        theta = np.random.multivariate_normal(
            [1, 1 + i], [[10, 0], [0, 1]], int(n_rounds / 2)
        )
...
def generate_bt(
    n_samples: int, sigma_sat: float, sigma_gpa: float, args: argparse.Namespace
    ...
    b[disadv_idx, 0] = np.random.normal(
        mean_sat_disadv, sigma_sat, b[disadv_idx][:, 0].shape
    )  # SAT
    b[disadv_idx, 1] = np.random.normal(
        mean_gpa_disadv, sigma_gpa, b[disadv_idx][:, 1].shape
    )  # GPA

    # advantaged students
    b[adv_idx, 0] = np.random.normal(
        mean_sat_adv, sigma_sat, b[adv_idx][:, 0].shape
    )  # SAT
    b[adv_idx, 1] = np.random.normal(
        mean_gpa_adv, sigma_gpa, b[adv_idx][:, 1].shape
    )  # GPA
    ...
    g = (
        np.ones((args.num_applicants, args.num_envs)) * 0.5
    )  # legacy students shifted up
    g[disadv_idx, :] = -0.5  # first-gen students shifted down
    g += np.random.normal(
        1, 0.2, size=(args.num_applicants, args.num_envs)
    )  # non-zero-mean
