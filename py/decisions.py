...
from py.agents_gen import (
    clip_outcomes,
    evaluate_agents,
    compute_percentile_admissions,
    compute_random_admissions,
    AgentsGenericModel,
)
...
class ThetaGenerator:
    ...
    def generate_randomly(self) -> np.ndarray:
        ...
        for i in range(n):
            thetas_tr[i] = np.random.multivariate_normal(
                [1, 1 + i], [[10, 0], [0, 1]], size=T
            )

        return np.stack(thetas_tr).transpose((1, 0, 2))  # (T,n,m)

    def generate_scaled_duplicates(
        self, deploy_sd_every: int, mean_shift: int = 0
    ) -> np.ndarray:
        """
        Args:
          deploy_sd_every (int):
        ...
        def _distribute(n_rounds, theta, theta_scaled, deploy_sd_every):
            ...
            for i in range(deploy_sd_every, int(n_rounds / 2), deploy_sd_every):
                ...
                theta_temp[
                    j + deploy_sd_every : j + deploy_sd_every + deploy_sd_every
        ...
        for i in range(n):
            theta = np.random.multivariate_normal(
                [1, 1 + i + mean_shift], [[10, 0], [0, 1]], int(T / 2)
            )
    ...
    def generate_general_coop_case(self, num_cooperative_principals: int) -> np.ndarray:
        ...
        thetas_tr = (
            ThetaGenerator(
                length=T, num_principals=num_cooperative_principals
        ...
        if num_free_principals >= 1:
            ...
            for i in range(num_free_principals):
                out = (
                    ...
                    .generate_scaled_duplicates(
                        deploy_sd_every=(2 + i), mean_shift=num_cooperative_principals
                    )
...
class Simulator:
    ...
    def deploy(
        self, thetas_tr: np.ndarray, gammas: Iterable, admission_rates: Iterable
    ) -> None:
        """
        Args:
        ...
        # (n,) and (n,T,m) -> (T,m)
        avg_theta_tr = (gammas.reshape((n, 1, 1)) * thetas_tr.transpose((1, 0, 2))).sum(
            axis=0
        )

        # release thetas & average thetas
        thetas_tr = ThetaGenerator.intra_round_repeat(
            thetas_tr=thetas_tr, repeats=s
        )  # (Txs,n,m)
        avg_theta_tr = ThetaGenerator.intra_round_repeat(
            thetas_tr=avg_theta_tr, repeats=s
        )  # (Txs,m)
        ...
        x_norm = (x_tr - x_tr.mean(axis=0, keepdims=True)) / x_tr.std(
        ...
        for i in range(n):
            for j in range(T * s):
                ...
                w[i, idx1:idx2] = selection_func(
                    y_hat=y_hat[idx1:idx2, i], p=admission_rates[i]
                )
