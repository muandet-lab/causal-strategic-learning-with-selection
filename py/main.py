...
def run_utility_exp():
    ...
    my_thetas_tr = ThetaGenerator(
        length=T, num_principals=1
    ).generate_randomly()  # (T,1,2)
    your_thetas_tr = np.tile([1, 1], reps=(T, 1, 1))  # (T,1,2)
    delpoyed_thetas_tr = np.concatenate(
        [my_thetas_tr, your_thetas_tr], axis=1
    )  # (T,2,2)
    ...
    sim.deploy(
        thetas_tr=delpoyed_thetas_tr, gammas=gammas, admission_rates=admission_rates
    )
    ...
    z = sim.z
    outputs = [
        y[i * s : (i + 1) * s][z[i * s : (i + 1) * s] == 1].mean() for i in range(T)
    ]
