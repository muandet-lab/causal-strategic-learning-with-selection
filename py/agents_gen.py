...
def normalise_agents(
    b_tr: np.ndarray, x_tr: np.ndarray, eet_mean: np.ndarray
    ...
    x_tr = copy.deepcopy(x_tr)
    (b_tr[:, 0], x_tr[:, 0]), scale1 = normalize(
        [b_tr[:, 0], x_tr[:, 0]], new_min=400, new_max=1600
    )
    (b_tr[:, 1], x_tr[:, 1]), scale2 = normalize(
        [b_tr[:, 1], x_tr[:, 1]], new_min=0, new_max=4
    )
...
class AgentsGenericModel:
    ...
    def __init__(
        self,
        group_0_base_mean: Iterable,
        group_0_base_cov: Iterable,
        group_1_base_mean: Iterable,
        group_1_base_cov: Iterable,
        group_0_outcome_mean_shift: float,
        group_0_outcome_std: float,
    ...
    def gen_base_agents(
        ...
        b_tr = (1 - u).reshape(-1, 1) * np.random.multivariate_normal(
            **b_dis_0, size=T
    ...
    @staticmethod
    def gen_covariates(
        b_tr: np.ndarray, e_stack: np.ndarray, avg_theta_tr: np.ndarray
    ) -> np.ndarray:
        """
    ...
    def gen_outcomes(
        self, u: np.ndarray, x_tr: np.ndarray, theta_stars_tr: np.ndarray
        ...
        o = (1 - u) * np.random.normal(**o_dis_0, size=T) + u * np.random.normal(
            **o_dis_1, size=T
        )
        o = np.tile(
            o.reshape(-1, 1), n
        )  # all envs have the same noise-generating mechanism
    ...
    def realise_enrollments(w_tr: np.ndarray, gammas_tr: np.ndarray) -> np.ndarray:
        ...
        probs_tr = skl_normalize(
            w_tr * gammas_tr, axis=1, norm="l1"
        )  # auto handles the zero case.
        ...
        for i in range(T):
            ...
            if p.sum() > 0:  # only for agents with at least 1 admission offer.
                z[i] = (
                    np.random.choice(n, p=p) + 1
                )  # offset to avoid conflict with "no uni" decision
